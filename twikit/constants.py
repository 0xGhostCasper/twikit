...
# ListLatestTweetsTimeline and ListMembers features.
# Sourced from fa0311/TwitterInternalAPIDocument (May 2026). The shared FEATURES
# dict is too narrow for these endpoints: Twitter returns an empty entries[] when
# new feature flags are missing.
...
ARTICLE_FEATURES = {
    # Matches the documented feature set for ArticleEntityResultByRestId
    # in fa0311/TwitterInternalAPIDocument (the auto-scraped x.com
    # web-client schema). The earlier hypothesis that more flags were
    # needed turned out to be wrong — the empty `result` we saw was
    # caused by a stale query ID (see Endpoint.ARTICLE_RESULT_BY_REST_ID).
    'profile_label_improvements_pcf_label_in_post_enabled': True,
    'responsive_web_profile_redirect_enabled': False,
    'rweb_tipjar_consumption_enabled': False,
    'verified_phone_label_enabled': False,
